...
// ── 文字サイズクラス制御（14px, 20px, 30pxマッピング） ──
function applyFontSize() {
  ...
  if (S.fontSize === 'small') {
    ...
    document.body.classList.add('fs-medium');
  }
}
        ...
        try {
          ...
          root.innerHTML = `
            <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;padding:32px;text-align:center">
              ...
              <div style="font-size:13px;color:rgba(45,27,0,0.5)">よみとりにしっぱいしたよ<br>もう一度ためしてね</div>
...
const App = {
  ...
  dismissStreakPop() {
    S.streakBrokenPop = false;
    S.streakBrokenCount = 0;
    render();
  },
  ...
  // 追加：お気に入り・ノートの追加20件読み込み
  loadMoreFav() {
    S.favLimit = (S.favLimit || 20) + 20;
    render();
  },
  loadMoreNotes() {
    S.noteLimit = (S.noteLimit || 20) + 20;
    render();
  },
