...
/** 保存対象データを組み立てる（persistSave / 全データエクスポートで共用） */
function _buildSaveData() {
  return {
    ...
    ownedStickyColors: S.ownedStickyColors ?? [],
    shopPurchaseCount: S.shopPurchaseCount ?? 0,
    devSupportActive:  S.devSupportActive  ?? false,
    claimedShopBadge:  S.claimedShopBadge  ?? false,
  };
}
...
function persistSave() {
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(_buildSaveData()));
...
/** 保存データを S へ反映する（persistLoad / 全データインポートで共用） */
function _applySaveData(saved) {
  Object.assign(S, {
    ...
    ownedStickyColors: saved.ownedStickyColors ?? [],
    shopPurchaseCount: saved.shopPurchaseCount ?? 0,
    devSupportActive:  saved.devSupportActive  ?? false,
    claimedShopBadge:  saved.claimedShopBadge  ?? false,
  });
}
